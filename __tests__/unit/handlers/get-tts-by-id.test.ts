...
import { id, joke, jokeWithAudio, synthesizeSpeechResult } from '../__mocks__'
...
describe('get-tts-by-id', () => {
  ...
  describe('getByIdHandler', () => {
    ...
    test('expect NOT_FOUND when getJokeById rejects', async () => {
    ...
    test('expect OK when no audio exists', async () => {
      ...
      expect(Buffer.from(result.body, 'base64').toString('utf8')).toEqual(joke.contents)
      expect(mocked(dynamodb).updateJoke).toHaveBeenCalledWith(id, jokeWithAudio, joke.version)
    })
