...
export const getPayloadFromEvent = (event: APIGatewayProxyEvent): Payload => {
  try {
    return JSON.parse(
      event.isBase64Encoded ? Buffer.from(event.body as string, 'base64').toString('utf8') : (event.body as string)
    )
  } catch (error) {
    return {}
  }
...
export const getListFromParameter = (event: APIGatewayProxyEvent, parameterName: string): string[] => {
  const availableParameters = event.queryStringParameters ?? {}
